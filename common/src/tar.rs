...
use nom::{Parser, bytes::complete::take, error::ParseError, multi::many};
extern crate alloc;
...
mod test {
    ...
    static SINGLE_TAR: &[u8] = include_bytes!("../single.tar");
    static DOUBLE_TAR: &[u8] = include_bytes!("../double.tar");
...
impl<'a> TarHeader<'a> {
    ...
    where
        E: ParseError<&'a [u8]>,
    {
        take(12usize).map(|buf| crate::oct2int(buf) as usize)
    }
